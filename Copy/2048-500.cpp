...
void add_rand_num() {
    ...
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < 4; ++j) {
            /* 定位待生成的位置 */
            if (board[i][j] == 0 && n-- == 0) {
                board[i][j] = (rand() % 10 ? 2 : 0 ); /* 生成数字2或4，生成概率为9:1 */
                return;
            }
        }
    }
...
void refresh_show()
{
            ...
            if (board[i][j] != 0) 
			{
                ...
                else 
				{
                    ...
                    for (k = 1; k < 20; ++k) {
                        n = n >> 1;
                        if (n == 1) {
                            printf(" 2^%d │", k); /* 超过四位的数字用2的幂形式表示，如2^13形式 */
                            break;
                        }
                    }
                }
    ...
    if (get_null_count() == 0) {
        ...
        /* 判断是否输掉游戏 */
        if (if_game_over) {
            printf("\r                       游 戏 结 束 !  再 玩 一 次 [Y/N]:     \b\b\b\b");
#ifdef _WIN32
